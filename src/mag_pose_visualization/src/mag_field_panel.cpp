#include "mag_pose_visualization/mag_field_panel.hpp"
...
namespace mag_pose_visualization
{
    ...
    MagneticFieldPanel::MagneticFieldPanel(QWidget *parent) : rviz::Panel(parent), nh_()
    {
        ...
        switch_topic_button_ = new QPushButton("(仅原始数据)");
        ...
        QStringList pose_headers = {"x (m)", "y (m)", "z (m)", "roll (rad)", "pitch (rad)", "yaw (rad)", "磁铁强度 (A·m²)"};
        ...
        // 初始化订阅（原始数据来自 mag_sensor_node，使用 MagSensorData 类型）
        magnetic_field_sub_ = nh_.subscribe<mag_sensor_node::MagSensorData>(
            "/magnetic_field/raw_data", 30, &MagneticFieldPanel::onMagneticFieldMsg, this);
        magnet_pose_sub_ = nh_.subscribe<mag_sensor_node::MagnetPose>(
            "/magnet_pose/predicted", 10, &MagneticFieldPanel::onMagnetPoseMsg, this);
    }
    ...
    void MagneticFieldPanel::onMagneticFieldMsg(const mag_sensor_node::MagSensorData::ConstPtr &msg)
    {
    ...
    // 不再支持 mag_pose_estimation::MagneticField，统一使用 MagSensorData

    void MagneticFieldPanel::onMagnetPoseMsg(const mag_sensor_node::MagnetPose::ConstPtr &msg)
    {
...
} // end namespace mag_pose_visualization

PLUGINLIB_EXPORT_CLASS(mag_pose_visualization::MagneticFieldPanel, rviz::Panel)
