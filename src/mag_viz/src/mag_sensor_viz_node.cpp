...
// For older ROS/TF2 headers, include both quaternion and transforms
#include <tf2_geometry_msgs/tf2_geometry_msgs.h>
...
public:
	...
	{
		nh_.param<std::string>("topic", topic_, std::string("/mag_sensor/raw_data"));
		...
		ROS_INFO_STREAM("[mag_sensor_viz] subscribing: " << topic_ << ", publishing: " << marker_topic_
...
private:
	...
	void onMsg(const mag_sensor_node::MagSensorData::ConstPtr& msg)
	{
		...
		if (did_tf) {
			...
			vin.vector.x = fx; vin.vector.y = fy; vin.vector.z = fz;
			try {
				geometry_msgs::TransformStamped T = tf_buffer_.lookupTransform(
						target_frame_, msg->header.frame_id, msg->header.stamp, ros::Duration(0.05));
				tf2::doTransform(vin, vout, T);
				fx = vout.vector.x; fy = vout.vector.y; fz = vout.vector.z;
			} catch (const std::exception& e) {
		...
		if (norm > 1e-9) { dx = fx / norm; dy = fy / norm; dz = fz / norm; }

		m.points.resize(2);
		m.points[0].x = 0.0; m.points[0].y = 0.0; m.points[0].z = 0.0;
		...
		visualization_msgs::MarkerArray arr; arr.markers.push_back(m);
		pub_.publish(arr);
	}
