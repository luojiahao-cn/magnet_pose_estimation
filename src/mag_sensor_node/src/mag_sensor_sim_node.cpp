...
{
public:
    explicit MagSensorSimNode(ros::NodeHandle &nh) : nh_(nh)
    {
...
private:
    ...
    void loadParams()
    {
        ...
        nh_.param("simulation_config/motion/static_position", static_pos, static_pos);
        nh_.param("simulation_config/motion/static_orientation", static_orient, static_orient);
    ...
    {
        magnet_pose_pub_ = nh_.advertise<mag_sensor_node::MagnetPose>("/magnet_pose/simulation", 25);
        magnetic_field_pub_ = nh_.advertise<mag_sensor_node::MagSensorData>("/magnetic_field/raw_data", 25);
    }
